...
- `.agents/AGENTS.md` — business rules and brand guidelines (authoritative source for business facts; keep in sync with `agente.md`, which duplicates similar content with additional menu/pricing detail — when updating one, check the other). There's also a root-level `AGENTS.md` that mirrors this file's content for Codex — keep both in sync when editing.
